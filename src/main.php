...
if (version_compare(PHP_VERSION, MIN_PHP_VERSION) === -1) {
  throw new Exception('Minimum require PHP version is: ' . MIN_PHP_VERSION);
}
...
spl_autoload_register(function ($class_name) use ($dir) {
  include_once  $dir . DIRECTORY_SEPARATOR . (false === strpos($class_name, 'Exception') ? 'lib' : 'exception') . DIRECTORY_SEPARATOR . $class_name . '.php';
});
unset($dir);
...
--target-dir=path/to/backup
  This is the path to the target directory where a backup is stored. The direction must be created. The argument is required to pass, and it has no default value.
  On each backup run, the script will create a backup-[datetime] directory and copy all required indexes to it. So target-dir represents the container of all your backups, and it's safe to run the script multiple times.

--config=path/to/manticore.conf | -c=path/to/manticore.conf
  Path to manticore config. This is optional and in case if it's not passed we use default one for your platform. It's used to get the host and port to talk with the Manticore daemon.

--indexes=index1,index2,...
  A semicolon-separated list of indexes is required to backup. If you want to backup all, just pass skip passing this argument to the script. You cannot give unexisting indexes in your database to this argument.

--compress
  Should we compress our indexers or not. The default – no. We use lz4 for compression.

--unlock
  In case if something went wrong or indexes are still in lock state we can run the script with this argument to unlock it all.
...
if (isset($args['version'])) {
  echo 'Manticore Backup Script version: ' . VERSION . PHP_EOL;
  echo 'Minimum PHP version required: ' . MIN_PHP_VERSION . PHP_EOL;
  exit(0);
}
...
switch (true) {
  ...
  default: // backup
    ...
    if (!OS::isWindows() && posix_getuid() !== 0) {
      echo PHP_EOL . 'WARNING: we couldn\'t fully preserve permissions of the files you\'ve backed up. Be careful when you restore from the backup or re-run the backup as root' . PHP_EOL;
    }
